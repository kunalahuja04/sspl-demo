...
export class LoginPageComponent implements OnInit {
  ...
  // Login Form Model
  tenants: string[] = [
    'SSPL001 — SSPL Bank (Production)',
    'SSPL002 — SSPL Corporate (Enterprise)',
    'SSPL003 — SSPL NRI Banking (Global)',
    'SSPL004 — SSPL Sandbox (UAT Testing)'
  ];
  ...
  notices: NoticeItem[] = [
    {
      id: 1,
      content: 'Customers can now deposit Income-Tax/Corporate Taxes using all Bank Debit Cards and Credit Cards under State Bank Payment Gateway.',
      date: '06 Jun 2026'
    },
    {
      id: 2,
      content: 'Use your Card PIN/OTP/CVV2 only at ATMs or Bank websites. Never share with anyone. SSPL Bank never asks for these details over phone or email.',
      date: '03 Jun 2026'
    },
    {
      id: 3,
      content: 'SSPL Bank introduces 24x7 RTGS transactions. Transfer amounts above ₹2 lakh instantly, any time, any day.',
      date: '28 May 2026'
    }
  ];

  ngOnInit(): void {
    // Generate session token to initialize the captcha
  ...
  useDemoCredentials(): void {
    this.selectedTenant = 'SSPL001 — SSPL Bank (Production)';
  ...
  onLoginSubmit(): void {
    ...
    if (!this.sessionTokenData) {
      this.alertMessage = { type: 'error', text: 'Active session token required. Generating new session...' };
      this.generateSessionAndCaptcha();
      return;
    }
    ...
    setTimeout(() => {
      ...
      this.alertMessage = {
        type: 'success',
        text: `Welcome back, ${this.customerId}! Redirecting to your dashboard...`
      };
